...
def add_softmax_cross_entropy_loss_for_each_scale(scales_to_logits,
  ...
  for scale, logits in six.iteritems(scales_to_logits):
    ...
    not_ignore_mask = tf.cast(tf.equal(scaled_labels, 0), dtype=tf.float32) * label0_weight + tf.cast(tf.equal(scaled_labels, 1), dtype=tf.float32) * label1_weight + tf.cast(tf.equal(scaled_labels, ignore_label), dtype=tf.float32) * ignore_weight
